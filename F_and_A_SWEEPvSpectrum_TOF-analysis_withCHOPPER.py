...
def main():
    ...
    tdSplit, tcSplit = splitTimes(tdGes, tcGes, interval, noOfFrequencies * noOfAmplitudes)
    ...
    for j in range(noOfAmplitudes):
        ...
        for i in range(noOfFrequencies):
            N_ROI_local.append(getCounts(M, tdSplit[j*noOfFrequencies+i], tcSplit[j*noOfFrequencies+i])) #N_ROI_local = Array with j th Amplitude and the frequency Sweep
    ...
    for i in WantedAmplitudes:
        plt.errorbar(x, N_ROI[int(i/Amplitude_Steps - Starting_Amplitude/Amplitude_Steps)], np.sqrt(N_ROI[int(i/Amplitude_Steps - Starting_Amplitude/Amplitude_Steps)]), fmt='.', label = f'Amplitude = {i}mV pp' , markersize = 10)
    ...
    plt.savefig( f'pf2data/Plots/V_and_A_Sweep_with_Chopper_run{run}_Amplitudes_{WantedAmplitudes}__M_{M}.png', dpi = 300 )
    plt.show()
...
noOfAmplitudes = 3
Amplitude_Steps = 50
Starting_Amplitude = 375
M = 0.8
fMin = 10000
fMax = 14000

WantedAmplitudes = [375, 425, 475]
main()
